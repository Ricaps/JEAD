...
__UNFINISHED_LABEL = [
    ...
    re.compile(r"FIXME|\bFIX\b\s+ME", flags=re.IGNORECASE),
    re.compile(r"TBD", flags=re.IGNORECASE),
    re.compile(r"to\s+be\s+done", flags=re.IGNORECASE),
    re.compile(r"refactor", flags=re.IGNORECASE),
    re.compile(r"\btemp\b|temporary", flags=re.IGNORECASE),
...
__CODE_COMMENT_POSITIVE_LABEL = [
    ...
    re.compile(
        r"\"\b([a-zA-Z_]\w*(\.[a-zA-Z_]\w*)+)\b\"", flags=re.IGNORECASE
    ),  # package string references
    re.compile(
        r"^package\s+\b([a-zA-Z_]\w*(\.[a-zA-Z_]\w*)+)\b;$", flags=re.IGNORECASE
...
def label_unfinished_comments(dataset_path: Path):
    ...
    labeling = StaticRuleLabeling(
        positive_rules=__UNFINISHED_LABEL,
...
def label_code_snippets_comments(dataset_path: Path):
    dataset = load_dataset(dataset_path)
    labeling = StaticRuleLabeling(
        positive_rules=__CODE_COMMENT_POSITIVE_LABEL,

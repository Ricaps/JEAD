...
class PathResolver:
    ...
    def __init__(self, server_config: ServerConfig) -> None:
        ...
        self._python_path = self.__get_os_specific_venv_folder(
            self._venv_path, "python"
        )
    ...
    @staticmethod
    def __get_os_specific_venv_folder(
        venv_path: AsyncPath, executable_name: str
    ) -> AsyncPath:
        if os.name == "nt":
    ...
    def __get_requirements_file_name(self) -> str:
        return (
            PathResolver.REQUIREMENTS_GPU_FILE
            if self.use_gpu
            else PathResolver.REQUIREMENTS_FILE
        )
